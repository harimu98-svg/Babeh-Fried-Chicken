...
const WAHA_ADMIN_GROUP = '6282121266056@c.us'; // Admin Babeh Fried Chicken
...
async function sendWhatsAppWAHA(phoneNumber, message) {
    try {
        ...
        // Format nomor WhatsApp (sama seperti di reservation.js)
...
function updateSummary(subtotal, shipping) {
    ...
    document.getElementById('order-subtotal').textContent = `Rp ${formatRupiah(subtotal)}`;
    document.getElementById('order-shipping').textContent = `Rp ${formatRupiah(shipping)}`;
    document.getElementById('order-total').textContent = `Rp ${formatRupiah(total)}`;
    document.getElementById('payment-total').textContent = `Rp ${formatRupiah(total)}`;
}
...
// js/order.js - TAMBAHKAN FUNGSI MODAL QRIS
...
function createQRISModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content" id="qris-modal-content" style="
            background: white;
            padding: 25px;
            border-radius: 12px;
            max-width: 400px;
...
function showQRISPaymentModal(orderData) {
    ...
    if (!content) {
        console.error('❌ QRIS modal content not found');
        return;
    }
    ...
    content.innerHTML = `
        <div style="position:relative; text-align:center; padding:10px 0;">
            ${isSelfHosted ? `
                <div style="position:absolute; top:-10px; right:-10px; background:#28a745; color:white; padding:4px 12px; border-radius:20px; font-size:0.6rem; font-weight:bold;">
                    SELF HOSTED
                </div>
            ` : ''}
            ...
            <div id="qris-status" style="margin:15px 0; padding:10px; background:#fff3cd; border-radius:8px; color:#856404; font-size:0.9rem;">
                ⏳ Menunggu pembayaran...
            </div>
            ...
            ${isSelfHosted ? `
                <p style="font-size:0.7rem; color:#28a745; margin-top:10px;">
...
window.manualCheckPayment = async function(orderNumber) {
    ...
    try {
        const { data, error } = await window.supabaseClient
            ...
            .select('qrisly_history_id, qrisly_status, payment_status')
        ...
        if (data?.qrisly_status === 'paid' || data?.payment_status === 'Pembayaran Berhasil') {
            showNotification('✅ Pembayaran berhasil!', 'success');
            closeQRISModal();
            showSuccessPage(orderNumber);
        } else if (data?.qrisly_status === 'expired') {
...
window.manualCheckPayment = async function(orderNumber) {
    ...
    try {
        const { data, error } = await window.supabaseClient
            .from('order_fried_chicken')
            .select('qrisly_history_id, order_number')
            .eq('order_number', orderNumber)
            .single();
        if (error) throw error;
        if (data?.qrisly_history_id) {
            const response = await fetch(`/.netlify/functions/qrisly-status?history_id=${data.qrisly_history_id}`);
            const result = await response.json();
            const status = result.data?.payment_status || result.status;
...
async function submitOrder(event) {
    ...
    try {
        ...
        orderData.environment = qrisResult.environment || 'sandbox';
